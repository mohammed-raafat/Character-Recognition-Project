    ...
    methods (Static)
        ...
        function output=predict(Model,sample,varargin)
            ...
            if nsample>2
                ...
                parfor i=1:nsample
                    [~,threshold]=SVMHelper.svmclassify(model{i},sample,varargin{:});
                ...
                output=SVMHelper.svmclassify(model,sample,varargin{:});
            end
        ...
        function [outclass,val] = svmclassify(svmStruct,sample, varargin)
            ...
            if ~isempty(sample)
                ...
                %     try
                [outclass,val] = SVMHelper.svmdecision(sample,svmStruct);
                %     catch ME
        ...
        function [Model,predicted] = classify(Sample,class,SampleTest)
            Model=SVMHelper.train(Sample,class);
            predicted=SVMHelper.predict(Model,SampleTest);
        end
    end
end